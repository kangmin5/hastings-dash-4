...
const TwoStepsV1 = () => {
  // ** State
  const [isBackspace, setIsBackspace] = React.useState<boolean>(false)
  ...
  const handleChange = (event: React.ChangeEvent, onChange: (...event: any[]) => void) => {
    if (!isBackspace) {
      onChange(event)
      ...
      <Controller
        ...
        render={({ field: { value, onChange } }) => (
          <Box
            ...
            onChange={(event: React.ChangeEvent) => handleChange(event, onChange)}
...
TwoStepsV1.getLayout = (page: React.ReactNode) => <BlankLayout>{page}</BlankLayout>
