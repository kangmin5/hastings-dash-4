...
import QuickQuotes, { QuickQuote } from 'app/sales/seller/quick-quotes'
import { allQuickQuotes } from 'app/sales/seller/quick-quotes/repository'
import { QuickQuoteGridColumns } from "app/technical-support/flyweight/"
...
const QuickPage: NextPage = ({post}: Parameters) => {
  ...
  const [tuples, setTuples] = React.useState<QuickQuote[]>([])

  const dispatch = useAppDispatch()
  const counsel = useAppSelector((state: RootState) => state.counsel)
  React.useEffect(() => {

    QuickQuotes.new().type().all().then( function (res :any) {
      setTuples(res.data.array)})

    dispatch(allQuickQuotes())
  }, [dispatch])
