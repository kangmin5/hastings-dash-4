...
import {
  selectItemCount, selectAllSimples,

} from 'app/quotes/org/simple-org/simple-selector'
import { getAllSimples } from 'app/quotes/org/simple-org/simple-thunk'
...
const SimplePage: NextPage = ({ post }: Parameters) => {
  ...
  const AllSimples = useSelector(selectAllSimples)
  ...
  React.useEffect(() => {
    ...
    dispatch(getAllSimples(undefined))
  }, [dispatch])
  ...
  const SimpleColumn = (): GridColDef[] => [
      {
        ...
        sortable: false,
        headerName: 'No',
        renderCell: ({ row }: CellType) => <Typography variant='body2'>{row.quad.id}</Typography>
      },
      {
        flex: 0.1,
        minWidth: 400,
        ...
        sortable: false,
        headerName: '제목',
        renderCell: ({ row }: CellType) => <Typography variant='body2'>{row.article.title}</Typography>
      },
      {
        flex: 0.05,
        minWidth: 150,
      ...
      },
      {
        ...
        headerAlign: 'center',
        renderCell({ row }: CellType) {
          return <Typography variant='body2'>{row.reply.hasReply}</Typography>
        }
      },
      {
        flex: 0.05,
        ...
        sortable: false,
        renderCell({ row }: CellType) {
          return <Typography variant='body2'>{row.person.phone}</Typography>
        }
      },
  
      {
        ...
        headerAlign: 'center',
        renderCell({ row }: CellType) {
          return <Typography variant='body2'>{row.reply.replier}</Typography>
        }
      },
      {
        flex: 0.05,
