...
// ** Types
import { Employee } from 'app/personnel/manager/employees'

interface EmployeeJwtInterface{
  id: number,
  role: string,
  password: string,
  fullName: string,
...
const users: EmployeeJwtInterface[] = [
  {
    id: 1,
    role: 'admin',
    password: '1234',
...
mock.onGet('/auth/me').reply((config: any) => {
  ...
  jwt.verify(token, jwtConfig.secret as string, (err, decoded) => {
    ...
    if (err) {
      ...
      // ** Get user that matches id in token
      const userData = JSON.parse(JSON.stringify(users.find((u: EmployeeJwtInterface) => u.id === userId)))
