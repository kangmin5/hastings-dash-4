...
import { DepartDao as dao } from './depart-dao'
import { addDepart, getAllDeparts, getDepartsBy, getDepartById, alterDepartById, delDepartById } from './depart-thunk'
...
const DepartSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(addDepart.fulfilled, (state, { payload }) => {
        console.log('자주묻는 질문 게시판 프론트에서는 추가하기 기능이 없습니다. ')
      })
      .addCase(getAllDeparts.fulfilled, (state, { payload }) => {
      ...
      .addCase(getDepartsBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getDepartById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterDepartById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
