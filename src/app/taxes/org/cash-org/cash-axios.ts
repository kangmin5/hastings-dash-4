...
import { CashUrl as url } from './cash-union'
...
export interface CashType extends CrudAxios<CashVo> {}

export class CashAxios implements CashType {
  private static instance: CashAxios
  ...
  public static then(): CashAxios {
    if (!CashAxios.instance) {
      CashAxios.instance = new CashAxios()
    }

    return CashAxios.instance
  }

  addOne(parameter: CashVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: CashVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: CashVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: CashVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: CashVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: CashVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
