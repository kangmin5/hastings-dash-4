...
import { CardDao as dao } from './card-dao'
import { addCard, getAllCards, getCardsBy, getCardById, alterCardById, delCardById } from './card-thunk'
...
const CardSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(getCardsBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(alterCardById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delCardById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
