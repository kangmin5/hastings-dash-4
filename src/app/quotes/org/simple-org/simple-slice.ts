...
import { SimpleDao as dao } from './simple-dao'
import { addSimple, getAllSimples, getSimplesBy, getSimpleById, alterSimpleById, delSimpleById } from './simple-thunk'
...
const SimpleSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(addSimple.fulfilled, (state, { payload }) => {
        console.log('자주묻는 질문 게시판 프론트에서는 추가하기 기능이 없습니다. ')
      })
      .addCase(getAllSimples.fulfilled, (state, { payload }) => {
      ...
      .addCase(getSimplesBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getSimpleById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterSimpleById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delSimpleById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 삭제작업 : 쿼리 체크 Error `)
        }
      })
      .addCase(getAllSimples.rejected, (state, { payload }) => {
