...
import { QuickUrl as url } from './quick-union'
...
export interface QuickType extends CrudAxios<QuickVo> {}

export class QuickAxios implements QuickType {
  private static instance: QuickAxios
  ...
  public static then(): QuickAxios {
    if (!QuickAxios.instance) {
      QuickAxios.instance = new QuickAxios()
    }

    return QuickAxios.instance
  }

  addOne(parameter: QuickVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: QuickVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: QuickVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: QuickVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: QuickVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: QuickVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
