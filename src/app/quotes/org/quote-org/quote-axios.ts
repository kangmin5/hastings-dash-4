...
import { QuoteUrl as url } from './quote-union'
...
export interface QuoteType extends CrudAxios<QuoteVo> {}

export class QuoteAxios implements QuoteType {
  private static instance: QuoteAxios
  ...
  public static then(): QuoteAxios {
    if (!QuoteAxios.instance) {
      QuoteAxios.instance = new QuoteAxios()
    }

    return QuoteAxios.instance
  }

  addOne(parameter: QuoteVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: QuoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: QuoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: QuoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: QuoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: QuoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
