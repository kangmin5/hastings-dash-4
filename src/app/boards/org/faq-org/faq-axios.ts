...
import { FaqUrl as url } from './faq-union'
...
export interface FaqType extends CrudAxios<FaqVo> {}

export class FaqAxios implements FaqType {
  private static instance: FaqAxios
  ...
  public static then(): FaqAxios {
    if (!FaqAxios.instance) {
      FaqAxios.instance = new FaqAxios()
    }

    return FaqAxios.instance
  }

  addOne(parameter: FaqVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: FaqVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: FaqVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: FaqVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: FaqVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: FaqVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
