...
import { CourierDao as dao } from './courier-dao'
import {
  addCourier,
  getAllCouriers,
  getCouriersBy,
...
const CourierSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(getAllCouriers.fulfilled, (state, { payload }) => {
      ...
      .addCase(getCouriersBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getCourierById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterCourierById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delCourierById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 삭제작업 : 쿼리 체크 Error `)
        }
      })
      .addCase(getAllCouriers.rejected, (state, { payload }) => {
