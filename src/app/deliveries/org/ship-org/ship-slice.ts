...
import { ShipDao as dao } from './ship-dao'
import { addShip, getAllShips, getDeliveriesBy, getShipById, alterShipById, delShipById } from './ship-thunk'
...
const ShipSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(addShip.fulfilled, (state, { payload }) => {
        console.log('자주묻는 질문 게시판 프론트에서는 추가하기 기능이 없습니다. ')
      })
      .addCase(getAllShips.fulfilled, (state, { payload }) => {
      ...
      .addCase(getDeliveriesBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getShipById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterShipById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delShipById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 삭제작업 : 쿼리 체크 Error `)
        }
      })
      .addCase(getAllShips.rejected, (state, { payload }) => {
