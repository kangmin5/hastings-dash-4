...
import { BatchDao as dao } from './batch-dao'
import { addBatch, getAllBatches, getBatchesBy, getBatchById, alterBatchById, delBatchById } from './batch-thunk'
...
const BatchSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(getBatchesBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getBatchById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterBatchById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delBatchById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 삭제작업 : 쿼리 체크 Error `)
        }
      })
      .addCase(getAllBatches.rejected, (state, { payload }) => {
