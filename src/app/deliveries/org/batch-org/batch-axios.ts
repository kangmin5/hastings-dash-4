...
import { BatchUrl as url } from './batch-union'
...
export interface BatchType extends CrudAxios<BatchVo> {}
export class BatchAxios implements BatchType {
  private static instance: BatchAxios
  ...
  public static then(): BatchAxios {
    if (!BatchAxios.instance) {
      BatchAxios.instance = new BatchAxios()
    }

    return BatchAxios.instance
  }

  addOne(parameter: BatchVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: BatchVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: BatchVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: BatchVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: BatchVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: BatchVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
