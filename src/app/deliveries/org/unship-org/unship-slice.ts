...
import { UnshipDao as dao } from './unship-dao'
import { addUnship, getAllUnship, getUnshipBy, getUnshipById, alterUnshipById, delUnshipById } from './unship-thunk'
...
const UnshipSlice = createSlice({
  name: `UnshipItems`,
  initialState,
  reducers: {},
  extraReducers: builder => {
    builder
      .addCase(addUnship.fulfilled, (state, { payload }) => {
        console.log('자주묻는 질문 게시판 프론트에서는 추가하기 기능이 없습니다. ')
      })
      .addCase(getAllUnship.fulfilled, (state, { payload }) => {
      ...
      .addCase(getUnshipBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getUnshipById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterUnshipById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delUnshipById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 삭제작업 : 쿼리 체크 Error `)
        }
      })
      .addCase(getAllUnship.rejected, (state, { payload }) => {
        TD.rejectedTrouble(`[Reject Issue] ${DateMap().get()} 자주묻는 질문 전체목록 : 서버연결 실패 `)
      })
  }
})

export const {} = UnshipSlice.actions
export default UnshipSlice.reducer
