...
import { AttrUrl as url } from './attr-union'
...
export interface AttrType extends CrudAxios<AttrVo> {}
export class AttrAxios implements AttrType {
  private static instance: AttrAxios
  ...
  public static then(): AttrAxios {
    if (!AttrAxios.instance) {
      AttrAxios.instance = new AttrAxios()
    }

    return AttrAxios.instance
  }

  addOne(parameter: AttrVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: AttrVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: AttrVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: AttrVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: AttrVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: AttrVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
