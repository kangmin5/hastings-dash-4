...
import { CartDao as dao } from './cart-dao'
import { addCart, getAllCarts, getCartsBy, getCartById, alterCartById, delCartById } from './cart-thunk'
...
const CartSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(getCartsBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getCartById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterCartById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delCartById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
