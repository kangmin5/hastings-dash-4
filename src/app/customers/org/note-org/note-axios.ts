...
import { NoteUrl as url } from './note-union'
...
export interface NoteType extends CrudAxios<NoteVo> {}
export class NoteAxios implements NoteType {
  private static instance: NoteAxios
  ...
  public static then(): NoteAxios {
    if (!NoteAxios.instance) {
      NoteAxios.instance = new NoteAxios()
    }

    return NoteAxios.instance
  }

  addOne(parameter: NoteVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: NoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: NoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: NoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: NoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: NoteVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
