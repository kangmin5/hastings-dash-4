...
import { PointUrl as url } from './point-union'
...
export interface PointType extends CrudAxios<PointVo> {}
export class PointAxios implements PointType {
  private static instance: PointAxios
  ...
  public static then(): PointAxios {
    if (!PointAxios.instance) {
      PointAxios.instance = new PointAxios()
    }

    return PointAxios.instance
  }

  addOne(parameter: PointVo): Promise<AxiosResponse<any>> {
  ...
  getAll(parameter: PointVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getAll,
      method: get,
      data: null
  ...
  getBy(parameter: PointVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getBy,
      method: post,
      data: parameter
  ...
  getById(parameter: PointVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: getById,
      method: get,
      data: parameter
  ...
  alterById(parameter: PointVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: alterById,
      method: put,
      data: parameter
  ...
  delById(parameter: PointVo): Promise<AxiosResponse<any>> {
    const { method, url, headers, data }: AjaxVo = this.router.http({
      url: delById,
      method: del,
      data: parameter
