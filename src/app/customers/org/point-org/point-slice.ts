...
import { PointDao as dao } from './point-dao'
import {
  addPoint,
  getAllPoints,
  getPointsBy,
...
const PointSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(getAllPoints.fulfilled, (state, { payload }) => {
      ...
      .addCase(getPointsBy.fulfilled, (state, { payload }) => {
      ...
      .addCase(getPointById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 상세보기 : 쿼리 체크 Error `)
        }
      })
      .addCase(alterPointById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 수정작업 : 쿼리 체크 Error `)
        }
      ...
      .addCase(delPointById.fulfilled, (state, { payload }) => {
        if (TA.check().isGoodQuery(payload)) {
        } else {
          console.log(`[Error Issue] ${DateMap().get()} 자주묻는 질문 삭제작업 : 쿼리 체크 Error `)
        }
      })
      .addCase(getAllPoints.rejected, (state, { payload }) => {
