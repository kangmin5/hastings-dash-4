...
const FormLayoutsTabs = () => {
  // ** States
  const [value, setValue] = React.useState<string>('personal-info')
  ...
  // Handle Password
  const handlePasswordChange = (prop: keyof State) => (event: SelectChangeEvent<HTMLInputElement>) => {
  ...
  const handleConfirmChange = (prop: keyof State) => (event: SelectChangeEvent<HTMLInputElement>) => {
    setValues({ ...values, [prop]: event.target.value })
  }
