...
const UserViewSecurity = () => {
  // ** States
  const [defaultValues, setDefaultValues] = React.useState<any>({ mobile: '+1(968) 819-2547' })
  const [mobileNumber, setMobileNumber] = React.useState<string>(defaultValues.mobile)
  const [openEditMobileNumber, setOpenEditMobileNumber] = React.useState<boolean>(false)
  ...
  const handleConfirmNewPasswordChange = (prop: keyof State) => (event: React.ChangeEvent<HTMLInputElement>) => {
    setValues({ ...values, [prop]: event.target.value })
  }
